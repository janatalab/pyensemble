...
def select_study1(request,*args,**kwargs):
   ...
   # if it's an American advertisement, filter Canadian ads out of possible stims and proceed
    ...
    # Skip to the modality selection part (skip over lifetime period selection because there isn't one for
    # each lifetime period for the Canadian foils)
    if rand_region == 'Canada':
        ...
        # Select from among the least played stimuli 
        select_from_stims = possible_stims.filter(
            stimulusxattribute__attribute__name='Media Type',
...
def select_study1(request,*args,**kwargs):
   ...
   # if it's an American advertisement, filter Canadian ads out of possible stims and proceed
    if rand_region == 'USA':
        ...
        # What is the media type for the randomly selected stimulus for this age range?        
        media_type = StimulusXAttribute.objects.get(stimulus=stimulus, attribute = 1).attribute_value_text
